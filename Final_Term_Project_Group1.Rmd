---
title: "Final Term Project - IBM HR Analytics Employee Attrition & Performance"
author: "Group 1- Arrabe, Niemo, Pascual, Pulido"
date: "`r Sys.Date()`"
output:
...
departments <- df %>% group_by(Department, Gender) %>% summarise(amount=n()) %>%
ggplot(aes(x="", y=amount, fill=Department), show.legend=FALSE) + geom_bar(stat="identity", position="dodge") + theme_minimal() + 
theme(axis.text.x = element_text(angle = 90), plot.title=element_text(hjust=0.5), aspect.ratio=1) + 
labs(title="Number of Employees \n
by Department") + coord_polar() + scale_fill_manual(values=c("#FE642E", "#0080FF","#00FF40"))
...
plot_grid(gender.department, departments, ncol=2)
